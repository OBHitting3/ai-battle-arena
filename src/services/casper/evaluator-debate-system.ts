...
export interface Evaluation {
  ...
  improvements: string[];
  reasoning: string;
  quote: string; // A characteristic quote from this evaluator
}
...
export class EvaluatorDebateSystem {
  ...
  private evaluatorProfiles: Record<Evaluator, EvaluatorProfile> = {
  ...
  async runDebate(
    ...
    context: { niche: string; topic: string; purpose: string },
    maxRounds: number = 5
...
Be authentic to who you are. Speak in your characteristic style. Don't hold back your unique perspective.`;
   ...
   */
  private parseEvaluation(evaluator: Evaluator, evaluationText: string): Evaluation {
   ...
   */
  private async getAllEvaluations(
    content: string,
    context: { niche: string; topic: string; purpose: string }
  ): Promise<Evaluation[]> {
    const evaluators = [
   ...
   */
  private displayEvaluations(evaluations: Evaluation[]): void {
   ...
   */
  private async generateDebateTranscript(
    evaluations: Evaluation[],
    context: { niche: string; topic: string; purpose: string }
  ): Promise<string[]> {
...
Return ONLY the improved content, no explanations.`;
   ...
   */
  private generateDebateSummary(rounds: DebateRound[], consensusAchieved: boolean): string {
