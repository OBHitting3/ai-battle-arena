...
interface BeamSearchConfig {
  beamWidth: number;           // Number of candidate scripts (default: 3)
  minWordCount: number;         // Minimum script length (default: 300)
  maxWordCount: number;         // Maximum script length (default: 2000)
  targetDuration: number;       // Target video duration in seconds (default: 60)
...
export class BeamSearchScriptGenerator {
  ...
  async generate(
    videoIdea: VideoIdea,
    selectedNarrative: TreeOfThoughtNode,
    niche: string,
    channelDNA?: any
  ): Promise<{
    ...
    const candidatePromises = Array.from({ length: this.config.beamWidth }, (_, index) =>
      this.generateCandidate(videoIdea, selectedNarrative, niche, index, channelDNA)
    );
    ...
    const costTotal = rankedCandidates.reduce(
      (sum, candidate) => sum + (candidate.metadata.generationCost || 0),
      0
    );
  ...
  private async generateCandidate(
    videoIdea: VideoIdea,
    narrative: TreeOfThoughtNode,
    niche: string,
    index: number,
    channelDNA?: any
  ): Promise<BeamSearchCandidate> {
...
Write the complete script now. Make it unique and compelling.`;
   ...
   */
  private parseScript(content: string) {
...
Return ONLY a single number between 0.0 and 1.0, representing the overall predicted performance score.`;
   ...
   */
  private buildStyleInstructions(channelDNA: any): string {
    ...
    if (pacing) {
      ...
      instructions += pacingMap[pacing as string] || '';
    }
    ...
    if (vocabulary) {
      ...
      instructions += vocabMap[vocabulary as string] || '';
    }
   ...
   */
  convertToScript(
    candidate: BeamSearchCandidate,
    videoIdeaId: string,
    reasoningPath: 'creative' | 'factual'
  ): Partial<Script> {
