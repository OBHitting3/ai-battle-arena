...
export class AssetSourcer {
  private pexelsClient: ReturnType<typeof createClient>;
   ...
   */
  async searchPhotos(query: string, count: number = 10): Promise<AssetSearchResult[]> {
   ...
   */
  async searchVideos(query: string, count: number = 5): Promise<AssetSearchResult[]> {
    try {
   ...
   */
  async downloadAsset(asset: AssetSearchResult, fileName: string): Promise<string> {
    try {
  ...
  extractVisualKeywords(script: string): string[] {
    ...
    const visualTerms = words.filter(word =>
      word.length > 4 &&
   ...
   */
  async sourceAssetsForScript(script: string, assetsNeeded: number = 10): Promise<string[]> {
    try {
      const keywords = this.extractVisualKeywords(script);
      console.log(`Extracted keywords: ${keywords.join(', ')}`);

      const allAssets: AssetSearchResult[] = [];
 ...
 */
export function createAssetSourcer(apiKey: string): AssetSourcer {
