...
export class RemotionRenderer {
   ...
   */
  async renderVideo(composition: VideoComposition, outputFileName: string): Promise<string> {
    try {
   ...
   */
  private async createCompositionFile(composition: VideoComposition): Promise<string> {
...
`;
   ...
   */
  private async createVideoTemplate(composition: VideoComposition): Promise<void> {
    const templateCode = `
 ...
 */
export async function createCompositionFromScript(
  script: any,
  voiceoverPath: string,
  assetUrls: string[],
  ...
  const assets: VideoAsset[] = assetUrls.map((url, index) => ({
    type: 'image' as const,
    url,
